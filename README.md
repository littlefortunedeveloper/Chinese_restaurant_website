...
| 公告（便利贴墙） | `data/announcements.txt` 每行 `日期 \| 标题 \| 内容`，最新在上；展示为便利贴：黄/粉/绿轮换、微斜、胶带与折角、悬停摆正，标题/日期手写字体（在线加载、离线优雅回退）；**休假联动**：`CLOSURE_ENABLED: ON` 时自动置顶一张休假便签（日期签=区间、内容=CLOSURE 原文、标题 `CLOSURE_ANN_TITLE` 可改、`CLOSURE_ANN: OFF` 关联动），恢复营业自动消失 |
| 菜单搜索 | 分词 AND 匹配：查询按空格拆词、每个词命中即算（顺序无关），`with`≡`w.`、点逗号大小写无关；索引含 编号/英文/中文/西文/**分类名**，如 `chicken with broccoli lunch special` 直达午市特餐的 Chicken w. Broccoli；搜索激活时限时分类（午市）内的命中也会显示，清空后恢复时段规则；占位提示 `MENU_SEARCH_PLACEHOLDER` 可改 |
| 十格进度隔断 | 两条组标签上方各一行 10 枚矩形（绿=官网直订环、蓝=在线平台环的均值），每枚=10%，支持格内部分点亮（0.1% 精度，悬停显示百分比），与进度环同源、逐分钟同刷新；环总开关 OFF、官网手动 OFF、平台全 OFF 或组隐藏时对应行自动消失；打烊后/歇业日/休假中十格前亮**充电闪电**（SVG 充能+电流闪烁，回充相位自动亮灭，ORDER_BOLT: OFF 可关）|
| 满额赠送 SPECIAL DEALS | 总开关 `DEALS_ENABLED: ON`；每行一条 `DEAL_1: 门槛 \| 赠品 \| 范围 \| 有效期`（第3/4段可省）。范围：`direct`/`phone`/`online`/`all`/逗号清单（如 `doordash,phone`，票面自动生成范围徽章）；文字标记通用：`~~旧价~~`划线、`**新价**`红色醒目；编号可留空档；关开关或删光活动行=整块消失；有效期文字含日期（`8/31` 或 `2026-08-31`）则**最后有效日当天打烊时自动从网站消失**（今日截止的活动关门即下架；该日休息则次日0点；营业到凌晨的撑到实际打烊；按餐馆时区，跨年请写年份），最后一天票面自动亮出"⏳ Ends today · 今日截止"红标，无日期=纯展示；再加 `auto`/`manual` 关键词（顺序随意）票面显示"自动生效/下单时请告知"。`DEAL_LABEL` 改区标题；`ORDER_DIRECT_CARD_NOTE` 是官网直订下的银行卡提醒（支付提示，不随活动开关，留空隐藏） |
| 午市/晚市横幅 | `LUNCH_HOURS: 11:00 AM – 3:30 PM`（留空=关闭）。午市快结束时横幅倒计时（提前 `LUNCH_COUNTDOWN_MINUTES`（本站60）分钟开始，文案 `COUNTDOWN_LUNCH_END`，打烊倒计时优先），结束后显示 `COUNTDOWN_DINNER` 提示 `DINNER_NOTE_MINUTES`（默认90）分钟后自动隐藏 |
| 放假/临时歇业 | 双钥匙：`CLOSURE_ENABLED: ON` + `CLOSURE: 2026-07-13 to 2026-07-19 装修升级暂停营业 · CLOSED for Renovation · 7/21 恢复`。开始前 `CLOSURE_NOTICE_DAYS`（默认14）天起，顶部出现**独立的**琥珀色预告横幅（前缀 `CLOSURE_NOTICE_PREFIX` 可改），与红色打烊倒计时横幅分开、可同时叠放；歇业期间由预告横幅显示原文、倒计时横幅静默；官网直订变红不可点、第三方保持蓝色"可预订"、进度环按歇业时长从0%回充；过期自动恢复，开关 OFF 时日期不奏效；日期认 `2026-08-10`/`2026/8/10`/`8/10`（无年=今年），连接词 to/-/–/~/到 均可；英文月名与中文日期不识别（会成无限期歇业），请用数字日期 |
