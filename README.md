...
| 订餐按钮进度环 | 自动开启：环沿整个按钮外圈倒计时，从100%满格走向0%——绿环走空=该平台停止接单（DoorDash提前15分、其余30分、官网直订到打烊整点），"可预订"黄/蓝环走空=即将开门（跨休息日连续倒数），打烊中的官网直订红环同样倒数到开门；三色环与状态徽章同色，手动OFF的平台不带环。想关闭：在 site_config.txt 加一行 `ORDER_PROGRESS_RING: OFF` |
